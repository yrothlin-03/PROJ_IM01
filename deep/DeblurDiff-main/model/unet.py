...
class AttentionBlock(nn.Module):
    ...
    def grids(self, x):
        ...
        parts = th.cat(parts, dim=0)
        self.idxes = idxes
        return parts

    def grids_inverse(self, outs):
        preds = th.zeros(self.original_size).to(outs.device)
        b, c, h, w = self.original_size

        count_mt = th.zeros((b, 1, h, w)).to(outs.device)
        k1, k2 = self.kernel_size
        k1 = min(h, k1)
        k2 = min(w, k2)
                    ...
                    if not exists(num_attention_blocks) or nr < num_attention_blocks[level]:
                        layers.append(
                            AttentionBlock(
                                ...
                                use_checkpoint=use_checkpoint, kernel_size=(k, k)
                            )
                        )
