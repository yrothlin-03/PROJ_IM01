...
\begin{figure}[H]
    ...
    \label{fig:circ_vs_symm1}
\end{figure}    

Figure~\ref{fig:circ_vs_symm1} compares TV--deconvolution under circular and symmetric
boundary conditions when the ground-truth blur kernel is used in the forward model.
Contrary to the intuitive expectation that symmetric boundaries should better match
the physical acquisition process, our experiments show that the quantitative
performance is very similar for both choices, with PSNR values that are generally
comparable and sometimes slightly higher for circular convolutions. Moreover, we
observe more pronounced ringing artefacts with symmetric boundary conditions,
especially along strong edges. This suggests that, in our implementation, the
combination of symmetric extensions with the discrete convolution and TV
regularisation operators introduces additional oscillations near the borders,
which are then propagated into the image by the inverse filter. As a result, the
simple circular model, despite its unrealistic periodicity assumption, proves to
be at least as effective as the symmetric one from a purely quantitative (PSNR)
point of view. From a visual standpoint, however, the symmetric reconstructions
tend to preserve global contrast and the perceived separation between structures
in the central part of the image better than the circular ones, which sometimes
appear slightly more washed out because of the periodic continuation.
...
\begin{figure}[H]
    ...
    \label{fig:circ_vs_symm2}
\end{figure}   

Figure~\ref{fig:circ_vs_symm2} shows the same comparison when the blur kernel is no
longer assumed to be known but estimated using the Goldstein--Fattal procedure.
As expected, the overall reconstruction quality decreases for both types of
boundary conditions: slight inaccuracies in the estimated kernel translate into
residual blur and additional ringing artefacts. In this regime the choice of
boundary conditions remains important, but the behaviour is less clear-cut than
one might expect. In our experiments, circular convolutions often yield PSNR
values that are comparable to or slightly higher than those obtained with
symmetric boundaries, and the symmetric case tends to exhibit more visible
ringing. A plausible explanation is that the combination of kernel errors and
symmetric extensions leads to stronger oscillations at the borders, which are
then spread across the image by the inverse filter and only partially damped by
TV regularisation. From a perceptual point of view, however, the symmetric
reconstructions frequently preserve a higher overall contrast and a clearer
separation between foreground and background structures in the central area of
the image, whereas the circular case often looks slightly flatter and lower in
contrast despite its marginal PSNR advantage.
...
Figure~\ref{fig:noise_test} illustrates the behaviour of the TV--deconvolution
...
even extension of the image outside the domain. This avoids wrap--around artefacts and could in principle improve the modelling near
the borders, but in our experiments the resulting PSNR is at best comparable and
often slightly lower than in the circular case, and the symmetric reconstructions
tend to exhibit more ringing. Visually, however, they usually preserve a higher
contrast in the central region and can therefore look more pleasing than the more
greyish circular solutions at moderate noise levels.
...
\begin{figure}[H]
    ...
    \label{fig:hyperparam_test2}
\end{figure}

Figure~\ref{fig:hyperparam_test2}  shows a PSNR heatmap as a function of the two TV--deconvolution hyperparameters $(\lambda,\gamma)$. The regularisation weight $\lambda$ controls the trade--off between data fidelity and TV smoothing, while $\gamma$ is the penalty parameter that enforces the constraint $d = \nabla u$ in the split--Bregman scheme. 
...
experiments highlight that the Goldstein--Fattal estimator is reasonably stable for low noise levels but degrades quickly when the noise variance
...
the estimated kernels still exhibit an elongated diagonal structure, they approximate the true kernel better but only in the center. On the edges, the error is much more pronounced The corresponding deconvolved images are
...
Also, the blur kernel must be too large for the method to work properly.
...
Figure~\ref{fig:jpeg_robustness} illustrates the behaviour of the proposed
